...
function createGauges() {
	createGauge("boosterAcceleration",   "BST ACC", 0, 300,  "ft/s²" );	// booster acceleration
	...
	createGauge("sustainerAcceleration", "SUS ACC", 0, 300,  "ft/s²" );	// sustainer acceleration
} // end function createGauges()

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

/*
 * simple devtest function to set the gauge value
 */
...
function setGauges(currBstAcc, currBstVel, currThrust, currSusAcc, currSusVel) {
	gauges["boosterAcceleration"].redraw(currBstAcc);
	gauges["boosterVelocity"].redraw(currBstVel);
	gauges["thrust"].redraw(currThrust);
	gauges["sustainerVelocity"].redraw(currSusAcc);
	gauges["sustainerAcceleration"].redraw(currSusVel);
}
 ...
 */
//var gauge = module.exports = function(gaugeName, gaugeConfig) {
function Gauge(gaugeName, gaugeConfig) {
	...
	this.render = function() {
		...
		this.body = d3.select.document.getElementById(this.gaugeName)								//("#" + this.gaugeName)
		...
		if (undefined != this.config.label) {
			...
			this.body.append("svg:text")							// use predefined svg text config
				 ...
				 .text(this.config.label)								// label the gauge appropriately
		...
		if (undefined != this.config.units) {
			...
			this.body.append("svg:text")							// use predefined svg text config
				 ...
				 .attr("dy", fontSize / 2)							// offset distance		
				 .attr("text-anchor", "middle")					// center the text (css)
				 .text(this.config.units)								// label the units appropriately
				 .style("font-size", fontSize + "px")		// set font size (css)
		...
		pointerContainer.selectAll("path")
				...
				.style("stroke-width", "1px")						// needle edge weight
